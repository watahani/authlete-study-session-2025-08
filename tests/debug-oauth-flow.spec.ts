...
test.describe('Debug OAuth Flow', () => {
  ...
  test('debug oauth authorization flow step by step', async ({ page }) => {
    console.log('1. Starting OAuth flow debug test...');
    ...
    // ログイン後の状態を確認
    console.log('5. After login - waiting and checking URL...');
    await page.waitForTimeout(2000);
    console.log('Current URL after login:', page.url());
    await page.screenshot({ path: 'debug-step3-after-login.png' });
    
    // 2. OAuth認可エンドポイントにアクセス
    console.log('6. Going to OAuth authorize endpoint...');
    ...
    // OAuth認可後の状態を確認
    console.log('7. After OAuth authorize request - checking...');
    await page.waitForTimeout(3000);
    console.log('Current URL after OAuth request:', page.url());
